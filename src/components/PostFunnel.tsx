...
export function PostFunnel() {
  ...
  const highlightIndices = useMemo(() => {
    ...
    const stepMatches = searchResults.matches.filter(
      (m) => postFunnelSteps[m.stepIndex]?.id === activeStepData.id && m.fieldIndex >= 0
    )
    if (stepMatches.length === 0 && searchResults.matches.find(
      (m) => postFunnelSteps[m.stepIndex]?.id === activeStepData.id
    ) === undefined) {
      return []
    }
  ...
  const highlightStepTitle = useMemo(() => {
    ...
    return searchResults.matches.some(
      (m) => postFunnelSteps[m.stepIndex]?.id === activeStepData.id && m.fieldIndex === -1
    )
  }, [searchTerm, searchResults, activeStepData])
