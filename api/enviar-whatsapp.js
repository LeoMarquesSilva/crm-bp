...
export default async function handler(req, res) {
  ...
  try {
    ...
    const raw = typeof number === 'string' ? number.trim() : ''
    const msg = typeof text === 'string' ? text.trim() : ''

    /** Contato: só dígitos + DDI 55. Grupo WhatsApp: JID com @g.us (Evolution). */
    let numberFormatted = ''
    if (raw.includes('@')) {
      numberFormatted = raw.replace(/\s/g, '')
    } else {
      const num = raw.replace(/\D/g, '')
      if (num) numberFormatted = num.length <= 11 ? `55${num}` : num
    }

    if (!numberFormatted || !msg) {
      return res.status(400).json({
        error: 'Faltam parâmetros',
        message:
          'Envie number e text no body (JSON). number = telefone (DDD + número) ou JID do grupo (ex.: ...@g.us).',
      })
    }
